...
plot_obs_tot <- ggplot() +
  geom_sf(
    data = data_summary_per_obs_ALL_map$geometry,
...
plot_obs_harv <- ggplot() +
  geom_sf(
    data = data_summary_per_obs_HARV_map$geometry,
...
plot_scatter_all <- ggplot(richness_all, aes(x = raw_species, y = std_species)) +
  ...
  labs(
    title = "Number of records VS number of species per department (All plants)",
...
plot_scatter_harv <- ggplot(richness_harv, aes(x = raw_species, y = std_species)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +
  labs(
    title = "Number of records VS number of species per department (WHP)",
    x = "Total number of records per department",
    y = "Number of species per department"
  ) +
  theme_minimal(base_size = 16)
...
plot_freq_dpt <- ggplot(graph_data) +
  ...
  theme_minimal() +
  theme(
    legend.position = c(0.835, 0.9), legend.title = element_blank(),
    ...
    plot.margin = margin(0.4, 0, 1, 1, "cm")
