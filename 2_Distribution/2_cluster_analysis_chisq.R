...
names(df_silhouette_eval) <- c(
  "sil_width", "transformation", "distance",
  "n_clusters", "data"
)

df_eval <- as.data.frame(matrix(nrow = 0, ncol = 9))
names(df_eval) <- c(
  "transformation", "distance", "n_clusters", "data",
...
for (i in 1:length(list_abund_data)) { # loop over different datasets
  ...
  for (nclu in 3:10) {
    ...
    # Export :
    write.csv(clusters, paste0(
      "processed_data/clustering/", nclu,
  ...
  for (nclu in 3:10) {
    ...
    plot_clus <- ggplot() +
      geom_sf(
        data = departements_clus,
        aes(fill = clus_ward_cut), colour = "white"
      ) +
      ggtitle(
        label = "Clustering (chisq, no transformation)",
  ...
  png(
    filename = paste0(
      "processed_data/clustering/plots/clustering_chisq_",
