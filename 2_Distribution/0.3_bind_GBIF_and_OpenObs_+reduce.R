...
reduce.pres.data <- function(data, grid_raster) {
  ...
  filtered_xy <- dismo::gridSample(xy,
    rast(grid_raster),
    n = 1
  ) # selects 1 point per grid cell, input grid needs to be a raster ! works only with a 2 column dataframe in entry (xy) or a SpatialPoints object
