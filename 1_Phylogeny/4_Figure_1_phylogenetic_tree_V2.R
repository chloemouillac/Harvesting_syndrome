...
# most_harv <-  c("Lamiaceae", "Ranunculaceae", "Boraginaceae", "Caprifoliaceae", "Ericaceae", "Violaceae", "Pinaceae", "Polygonaceae", "Rosaceae", "Crassulaceae")
most_harv <-  c("Lami.", "Ranuncul.", "Boragin.", "Caprifoli.", "Eric.", "Viol.", "Pin.", "Polygon.", "Ros.", "Crassul.")
...
plot_tree <- ggtree(tree_data, layout="fan",
  ...
  scale_fill_manual(
    ...
    values=c(
      ...
      "#E5A94E",  # Mid color
      "#C26B51",  # Lighter blue for medium values
...
##### Plot tree with standard residuals from the chi2 analysis instead of the % of WHP ####
# Import chi2 results :
chi2 <- read.csv(here::here("3_Ethnobotany","processed_data", "overutilised_families.csv")) %>%
  select(FAMILLE, std_residuals)
chi2$FAMILLE <- sub("aceae", ".", chi2$FAMILLE)

# Join with tip data :
tip_data <- left_join(tip_data, chi2, by="FAMILLE")
# (to get NAs for missing families)
...
plot_tree <- ggtree(tree_data, layout="fan",
  ...
  geom_fruit(
    ...
    mapping=aes(
      x=log10(number_harvested+0.2), # +0.2 to not have problems with infinite value of log(0) 
      y=FAMILLE,
      fill=cut(std_residuals, breaks=cuts)),
    color="white",
    stat="identity",
    orientation="y",
    offset = 0.47,
...
pdf(file = "plots/Fig_1_phylo_std_res.pdf", width = 50, height = 48)
plot_tree
dev.off()
